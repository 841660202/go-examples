...
func main() {
	...
	if err != nil {
		...
		// 只有当您的代码可能产生无效值进行验证时，才需要进行此检查，例如具有nil值的接口，包括我在内的大多数人通常没有这样的代码。
		...
		for _, err := range err.(validator.ValidationErrors) {
			...
			fmt.Println("日志输出indent：")
			// fmt.Println(indent) // byte
...
// 注意：您可能会问，为什么我不在验证器之外这样做，因为这样做可以直接连接到验证器中，并且您可以与验证标记组合，并且仍然有一个常见的错误输出格式。
func UserStructLevelValidation(sl validator.StructLevel) {

	user := sl.Current().Interface().(User)
	fmt.Printf("user: %+v\n\n", user)
	if len(user.FirstName) == 0 && len(user.LastName) == 0 {
		fmt.Println("错误日志输出UserStructLevelValidation start：")
		sl.ReportError(user.FirstName, "fname", "FirstName", "fnameorlname", "")
		sl.ReportError(user.LastName, "lname", "LastName", "fnameorlname", "")
		fmt.Println("错误日志输出UserStructLevelValidation end：")
	}
